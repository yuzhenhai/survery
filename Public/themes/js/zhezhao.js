...
function PDF_launch(ob, zzw, zzh, callback, titlestr) {
    ...
    var titleHeight = 0;
    if (!titlestr) {
        PDF_c.innerHTML = "<a style='background:url("+url+"/images/bt_closed.gif) no-repeat;width:30px;height:30px;margin:-10px -18px 0 0;display:inline;position:relative;float:right;cursor:pointer;' onclick='PDF_close();return false;'></a>";
    } else {
        if (titlestr == "no") {
            PDF_c.innerHTML = "";
        } else {
            var bkcolor = "#fafafa";
            var color = "#222";
            var bg = "background:url("+url+"/images/icon_popup_close.png) no-repeat;width:27px; height:27px;margin-top:5px;";
            if (needDefine) {
                bkcolor = "#0073d9";
                color = "#ebf5ff";
                bg = "background:url("+url+"/images/icon_popup_closenew.png) no-repeat;width:17px; height:17px;margin-top:10px;";
            }
